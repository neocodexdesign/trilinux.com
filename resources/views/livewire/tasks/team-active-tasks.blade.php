    ...
    <div class="flex-1 overflow-y-auto p-6">
        @forelse($groupedTasks as $projectIndex => $projectGroup)
            <div class="mb-6 last:mb-0" x-data="{ projectOpen: true }">
                <button @click="projectOpen = !projectOpen" class="mb-3 flex w-full items-center gap-3 border-b border-purple-500/20 pb-3 text-left transition-colors hover:border-purple-500/30">
                ...
                @foreach($projectGroup['stages'] as $stageGroup)
                    <div class="mb-4 ml-6" x-data="{ stageOpen: true }">
                        <button @click="stageOpen = !stageOpen" class="mb-3 flex w-full items-center gap-2 text-left transition-colors hover:text-purple-200">
                        ...
                        <div x-show="stageOpen" x-collapse class="ml-6 space-y-3">
                            @foreach($stageGroup['tasks'] as $task)
                                ...
                                @endphp
                                <div class="group relative overflow-hidden rounded-lg bg-{{ $statusColor }}-950/20 shadow-sm backdrop-blur-sm transition-all hover:bg-{{ $statusColor }}-950/30">
