    ...
    <div class="flex-1 overflow-y-auto p-6">
        @forelse($groupedTasks as $projectIndex => $projectGroup)
            <div class="mb-6 last:mb-0" x-data="{ projectOpen: true }">
                <button @click="projectOpen = !projectOpen" @dblclick.stop="$wire.dispatch('openProjectEditor', { projectId: {{ $projectGroup['project']->id ?? 0 }} })" class="mb-3 flex w-full items-center gap-3 border-b border-emerald-500/20 pb-3 text-left transition-colors hover:border-emerald-500/30">
                ...
                @foreach($projectGroup['stages'] as $stageGroup)
                    <div class="mb-4 ml-6" x-data="{ stageOpen: true }">
                        <button @click="stageOpen = !stageOpen" @dblclick.stop="$wire.dispatch('openStageEditor', { stageId: {{ $stageGroup['stage']->id ?? 0 }} })" class="mb-3 flex w-full items-center gap-2 text-left transition-colors hover:text-emerald-200">
                        ...
                        <div x-show="stageOpen" x-collapse class="ml-6 space-y-3">
                            @foreach($stageGroup['tasks'] as $task)
                                <div class="group relative overflow-hidden rounded-lg bg-emerald-950/20 shadow-sm backdrop-blur-sm transition-all hover:bg-emerald-950/30 cursor-pointer"
